  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [trafficRes, requestsRes, blockedRes, threatsRes] =
          await Promise.all([
        ...
        if (trafficRes.data && trafficRes.data.traffic) {
          setThreatData(
            trafficRes.data.traffic.map((item) => ({
              label: item.label,
              total: item.total,
        ...
        setThreatStats({
          ...
          activeThreats:
            requestsRes.data.totalRequests - blockedRes.data.blockedThreats,
        });
        ...
        <div className="chart-container">
          <ResponsiveContainer width="100%" height={300}>
            ...
            >
              ...
              <XAxis
                dataKey="label"
                tick={{ fontSize: 10, fill: "#b2becd" }}
                minTickGap={15}
              />
              <YAxis
                tick={{ fontSize: 12, fill: "#b2becd" }}
                allowDecimals={false}
                width={40}
                axisLine={false}
              />
              ...
              <div key={index} className="type-item">
                ...
                <div className="type-bar">
                  <div
                    ...
                    style={{
                      width: `${
                        (type.count / threatStats.totalThreats) * 100
                      }%`,
                    }}
                  ></div>
                </div>
